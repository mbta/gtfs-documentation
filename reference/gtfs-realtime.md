...
| [`EntitySelector`][mes]    | `facility`         | string                      | Many        | Optional | IDs of the facilities impacted by an alert (from [facilities.txt](gtfs.md#facilitiestxt) in GTFS). Included along with `activities` when an alert affects a specific facility of a stop, such as an elevator, escalator, or parking area.
